...
# --- 10. APLICACIÓN PRINCIPAL (USUARIO LOGUEADO) ---
  ...
  # ==========================================
  # PESTAÑA 3: ASISTENTE VIRTUAL IA (CON SELECCIÓN DE MODO: GUIADO / CONVERSACIONAL)
  # ==========================================
  with tab_asistente:
    ...
    if "tipo_asistente_activo" not in st.session_state:
      st.session_state["tipo_asistente_activo"] = "Chat Guiado"
    ...
    # --- SECCIÓN DE BOTONES SUPERIORES PARA CAMBIAR ENTRE MODO GUIADO Y CONVERSACIONAL ---
    c_btn1, c_btn2, c_btn_reset = st.columns([2, 2, 1])
    
    with c_btn1:
      if st.button("🧭 Chat Guiado", use_container_width=True, type="primary" if st.session_state["tipo_asistente_activo"] == "Chat Guiado" else "secondary"):
        st.session_state["tipo_asistente_activo"] = "Chat Guiado"
        st.session_state["modo_asistente"] = "menu_principal"
        st.rerun()

    with c_btn2:
      if st.button("💬 Chat Conversacional", use_container_width=True, type="primary" if st.session_state["tipo_asistente_activo"] == "Chat Conversacional" else "secondary"):
        st.session_state["tipo_asistente_activo"] = "Chat Conversacional"
        st.rerun()
    ...
    # ==========================================
    # OPCIÓN A: CHAT GUIADO
    # ==========================================
    if st.session_state["tipo_asistente_activo"] == "Chat Guiado":
    ...
    # ==========================================
    # OPCIÓN B: CHAT CONVERSACIONAL LIBRE
    # ==========================================
    else:
      st.markdown("### 💬 Chat Conversacional con la IA")
      st.write("Escribe libremente cualquier consulta sobre tus materias, notas o recomendaciones de estudio:")
      ...
      with st.form(key="form_chat_libre", clear_on_submit=True):
        pregunta_usuario = st.text_input("Escribe tu mensaje o pregunta aquí...")
        enviar_pregunta = st.form_submit_button("Enviar Mensaje 🚀")

        if enviar_pregunta and pregunta_usuario.strip():
          hora_msg = datetime.datetime.now().strftime("%I:%M %p").lower().replace("am", "a. m.").replace("pm", "p. m.")
          st.session_state["mensajes_asistente"].append({
              "role": "user",
          ...
          with st.spinner("Pensando respuesta..."):
            try:
              ...
              contexto_pensum = st.session_state.get("pensum_df").to_json() if st.session_state.get("pensum_df") is not None else "Sin pensum cargado"
              contexto_evals = json.dumps(st.session_state.get("evaluaciones", {}))
              contexto_horario = st.session_state.get("horario_df").to_json() if st.session_state.get("horario_df") is not None else "Sin horario cargado"

              prompt_sistema = f"""
              Eres un asistente virtual universitario experto, amigable y preciso.
              Ayudas al estudiante con base en sus datos académicos actuales:
              - Pensum y estado de materias: {contexto_pensum}
              - Evaluaciones y notas: {contexto_evals}
              - Horario de clases: {contexto_horario}

              Responde de manera clara, concisa y útil a la siguiente pregunta del estudiante:
              {pregunta_usuario}
              """

              respuesta_gemini = client.models.generate_content(
                  model=modelo_seleccionado, contents=prompt_sistema
              )

              texto_respuesta = respuesta_gemini.text if respuesta_gemini and respuesta_gemini.text else "No pude procesar una respuesta en este momento."

            except Exception as err:
              texto_respuesta = f"Ocurrió un error al conectar con Gemini: {err}"
    ...
    # Historial visual inferior para el modo guiado también
    if st.session_state["tipo_asistente_activo"] == "Chat Guiado":
      st.markdown("---")
      st.markdown("#### 💬 Historial de Interacciones Recientes")
      chat_html_guiado = '<div class="whatsapp-container">'
      for mensaje in st.session_state["mensajes_asistente"]:
        if mensaje["role"] == "user":
          chat_html_guiado += f'<div class="whatsapp-msg-user"><b>Tú:</b><br>{mensaje["content"]}</div>'
        else:
          chat_html_guiado += f'<div class="whatsapp-msg-assistant"><b>Asistente:</b><br>{mensaje["content"]}</div>'
      chat_html_guiado += '</div>'
      st.markdown(chat_html_guiado, unsafe_allow_html=True)
